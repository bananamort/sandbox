...
- `stepRCC()` (line 174):
  1. Backpressure guard: if `requestCount > 100`, throws `std::runtime_error` embedding all thirteen extern op counters (`requestCount`, `diagCount`, `batchJobCount`, `openJobCount`, `closeJobCount`, `helloWorldCount`, `getVersionCount`, `renewLeaseCount`, `executeCount`, `getExpirationCount`, `getStatusCount`, `getAllJobsCount`, `closeExpiredJobsCount`, `closeAllJobsCount`).
