...
Implements the Windows crash/hang reporter: installs a top-level `UnhandledExceptionFilter` (patching out CRT's `SetUnhandledExceptionFilter` reset via an x86 jmp hook and disabling the 64-bit user-mode callback exception filter), writes MiniDump+/Full-memory dumps via `MiniDumpWriteDump`, runs dump collection on a watcher thread (so stack overflows are capturable), detects 3-minute heart-beat hangs (`NotifyAlive` arms reporting; `DisableHangReporting` turns it off), and relaunches the exe with `-d` to upload dumps (skipped for RCCService).
...
- Hang-path dump call is `ProcessException(GetExceptionInformation(), true /*silent*/)` — the `true` is the **noMsg** parameter, *not* writeFullDmp; hang dumps are silent but still subject to the same full-dump sampling (`rand()%100 < DFInt::WriteFullDmpPercent`), and only fire once per session (`deadlockCounter++ == 0`).
