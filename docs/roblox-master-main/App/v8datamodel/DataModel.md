...
- Plugin: `GetJobsInfo`, `GetJobsExtendedStats`, `GetJobTimePeakFraction(jobname,greaterThan)`, `GetJobTimePeakFraction`, `SetPlaceVersion`, `SetPlaceId`+deprecated `SetPlaceID`, `SetGameInstanceId`, `SetUniverseId`, `SetCreatorId`+deprecated `SetCreatorID(creatorId,creatorType)`, `SetGenre`, `SetGearSettings(genreRestriction,allowedGenres)`.
