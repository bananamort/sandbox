...
- Ui messaging: SetUiMessage(msgType,uiMessage) Security::LocalUser; GetUiMessage / UiMessageChanged event — RobloxScript. Deprecated trio SetErrorMessage(LocalUser)/GetErrorMessage/ErrorMessageChanged aliased onto the same storage via Attributes::deprecated.
