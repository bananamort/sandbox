...
| CLIMBING | JUMP_CMD→JUMPING; TIPPED→FALLING_DWN; AWAY_LDR→RUNNING; OFF_FLOOR/ON_FLOOR/TOUCHED stay CLIMBING; HAS_BUOYANCY→SWIMMING |
...
Every non-DEAD state also exits to DEAD on NO_HEALTH/NO_NECK, SEATED on SIT_CMD, PLATFORM_STANDING on PLATFORM_STAND_CMD.
