...
- Pan/Tilt: `onMousePan(const Vector2& wrapMouseDelta)`, `onMouseTrack(...)`; `panRadians(float)/panUnits(int)/panSpeedRadians(float)/getPanSpeed()`; `tiltRadians(bool-returning float arg up)/tiltUnits(int)/tiltSpeedRadians(float)/getTiltSpeed()`.
