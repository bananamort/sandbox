...
- Asset loading: `loadAsset(int id, resume(shared_ptr<Instance>), error)` / `loadAssetVersion(id, ...)` / `getLatestAssetVersion(id, resume(int), error)`; `void insert(shared_ptr<Instance>)`; `void internalDelete(shared_ptr<Instance>)`.
