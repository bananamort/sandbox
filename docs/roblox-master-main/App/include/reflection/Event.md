...
- `template<EventClass, Signature, SignalType=rbx::signal<Signature>, SignalGetter=member-ptr> class EventDesc : public EventDescImpl<arity,...>` — "This is the final class used for defining Events." Ten ctors covering arities 0–7 (FIXED: doc previously claimed twelve): arity 0 and arity 1 each have 2 overloads (Security+Attributes / Attributes-only); arities 2–7 have exactly one (security defaults to None, attributes has a default). Each static-asserts exact arity and pushes named SignatureDescriptor Items.
