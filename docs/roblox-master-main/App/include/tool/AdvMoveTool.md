...
- `class AdvMoveToolBase : public AdvArrowToolBase`
  ...
  - Overrides: full input set + `render2d/render3dAdorn`, `getCursorName`, `setCursor(std::string)`; `drawConnectors()` overridden to `true` (header comment: default mouse command draws none); pure virtuals `getHandleColor()` / `getDragType()`.
