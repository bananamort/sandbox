...
- `class SimBody : public Allocator<SimBody>`
  ...
  - Also declared: `inline void updateMomentRecipWorld();`, `inline Vector3 computeRotationVelocityFromMomentum();` + `...Fast()` variant, `inline const Matrix3& getInverseInertiaInWorld() const` (ref to `momentRecipWorld`), `inline const PV& getPV() const`, `PV getOwnerPV();` (by value), private `void update();` (the dirty-refresh behind `updateIfDirty`).
