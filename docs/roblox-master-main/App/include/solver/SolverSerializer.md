...
- `serializeForces` iterates `simBodies.size()` then zero-fills to `total` — if callers pass `total < simBodies.size()`, extra bodies are silently dropped (loop bound `i` starts at simBodies.size()).
- Typo is API surface: `getRotationallmpulse` (double-l / l-vs-I), see [v8kernel/SimBody.h](../v8kernel/SimBody.md).
- Output path depends on `boost::filesystem::temp_directory_path()` and embeds userId — one file per user id per machine, appended across sessions (fileOpened resets only on close).
